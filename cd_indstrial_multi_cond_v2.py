"""
cd_indstrial_multi_cond_v2.py

完整的工业级条件变分自编码器 (CondCDVAE) - 强化训练版
特性：
1. [多条件] 支持任意数量的条件输入，自动识别属性维度 (如 Energy, Bandgap)
2. [防过拟合] 自动按 8:2 随机划分训练集 (Train) 和验证集 (Val)
3. [早停机制] 实时监控 Val Loss，自动保存最佳模型权重 (Early Stopping)
4. [物理约束] 包含 GNN 图编码器与原子防重叠斥力惩罚 (Repulsion Loss)
"""
...
class CrystalDataset(Dataset):
    def __init__(self, root_dir: str, id_prop_csv: str = "id_prop.csv"):
        ...
        if not os.path.exists(csv_path):
            raise FileNotFoundError(f"找不到 {csv_path}。请先运行 10k 下载脚本。")
...
# ==========================================
# 2. 增强型模型组件 (GNN Encoder & MLP Decoder)
# ==========================================
...
if __name__ == "__main__":
    ...
    parser.add_argument("--patience", type=int, default=15, help="如果连续 N 个 epoch 验证集 Loss 没降，则早停")
    
    parser.add_argument("--target_props", type=float, nargs='+', default=[-2.0, 1.5], 
                        help="输入的多个目标属性值。例如：-2.0 1.5")
    
    parser.add_argument("--r_cut", type=float, default=1.0)
    parser.add_argument("--lambda_rep", type=float, default=5.0)
    parser.add_argument("--save_dir", type=str, default="checkpoints", help="模型权重保存路径")
    args = parser.parse_args()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"🚀 使用的计算设备: {device}")
    
    # 1. 加载数据并自动切分 (80% Train, 20% Val)
    ...
    # 使用 PyTorch 原生的随机切分，保证完全随机且不重叠
    train_dataset, val_dataset = random_split(dataset, [train_size, val_size])
    
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, collate_fn=collate_fn)
    val_loader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, collate_fn=collate_fn)
    
    print(f"🔪 数据切分完毕: 训练集 {train_size} 个，验证集 {val_size} 个")
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_loader:
            ...
            train_rep_loss += rep_loss_val
            
        avg_train_loss = train_loss / len(train_loader)
        avg_train_rep = train_rep_loss / len(train_loader)
        ...
        with torch.no_grad(): # 验证集不计算梯度！
            for batch in val_loader:
                loss, rep_loss_val = compute_batch_loss(model, batch, device, r_cut=args.r_cut, lambda_rep=args.lambda_rep)
                val_loss += loss.item()
                val_rep_loss += rep_loss_val
                
        avg_val_loss = val_loss / len(val_loader)
        avg_val_rep = val_rep_loss / len(val_loader)

        print(f"Epoch [{epoch+1:03d}/{args.epochs}] | Train Loss: {avg_train_loss:.4f} (斥力:{avg_train_rep:.4f}) | Val Loss: {avg_val_loss:.4f} (斥力:{avg_val_rep:.4f})")
        ...
        if avg_val_loss < best_val_loss:
            ...
            print(f"   🌟 发现最佳模型！Val Loss 降至 {best_val_loss:.4f}，已自动保存权重。")
        else:
            epochs_no_improve += 1
            print(f"   ⚠️ Val Loss 未降低，耐心值消耗: {epochs_no_improve}/{args.patience}")

        if epochs_no_improve >= args.patience:
            print(f"\n🛑 早停触发！连续 {args.patience} 个 Epoch 验证集没有提升。训练提前结束。")
            break
    ...
    # 🌟 关键：生成前，把刚刚保存的最佳权重读回模型！
    print(f"✅ 正在加载整个训练过程中最优秀的权重: {best_model_path}")
    model.load_state_dict(torch.load(best_model_path))
    
    if len(args.target_props) != cond_dim:
        print(f"⚠️ 警告: 你输入了 {len(args.target_props)} 个参数，但数据集需要 {cond_dim} 个！将自动截断或补零...")
        targets = (args.target_props + [0.0] * cond_dim)[:cond_dim]
    else:
        targets = args.target_props
        
    print(f"设定的目标物理属性向量 (形成能, 带隙等): {targets}")
